...
class SearchPanel(ctk.CTkFrame):
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(self, text="资源探索", font=("Microsoft YaHei", 18, "bold"), text_color=self.colors['text_primary']).grid(row=0, column=0, pady=(15, 0), padx=20, sticky="w")
        
        search_frame = ctk.CTkFrame(self, fg_color="transparent")
        search_frame.grid(row=1, column=0, padx=20, pady=10, sticky="ew")
        search_frame.grid_columnconfigure(0, weight=1)
        
        self.search_entry = ctk.CTkEntry(search_frame, placeholder_text="输入关键字开始搜索...", height=40, font=("Microsoft YaHei", 14), fg_color=self.colors['item'], border_width=0, corner_radius=8, text_color=self.colors['text_primary'])
        self.search_entry.grid(row=0, column=0, sticky="ew", padx=(0, 5))
        self.search_entry.bind('<Return>', lambda e: self.start_search(1))
        
        self.paste_btn = ctk.CTkButton(search_frame, text="📋", width=30, height=40, font=("Arial", 16), fg_color=self.colors['item'], text_color=self.colors['text_secondary'], hover_color=self.colors['item_selected'], corner_radius=8, command=self._quick_paste)
        self.paste_btn.grid(row=0, column=1, padx=(0, 5))
        
        self.clear_btn = ctk.CTkButton(search_frame, text="✕", width=30, height=40, font=("Arial", 16), fg_color=self.colors['item'], text_color=self.colors['text_secondary'], hover_color=self.colors['item_selected'], corner_radius=8, command=lambda: self.search_entry.delete(0, 'end'))
        self.clear_btn.grid(row=0, column=2, padx=(0, 10))
        
        self.search_btn = ctk.CTkButton(search_frame, text="搜索", width=80, height=40, font=("Microsoft YaHei", 14, "bold"), fg_color=self.colors['accent'], hover_color=self.colors['accent_hover'], corner_radius=8, command=lambda: self.start_search(1))
    ...
    def start_search(self, page=1):
        ...
        ctk.CTkLabel(self.list_frame, text="正在连接服务器...", font=("Microsoft YaHei", 14), text_color=self.colors['text_secondary']).pack(pady=40)
    ...
    def update_list_ui(self, results, total_pages, err):
        ...
        if not results:
            ctk.CTkLabel(self.list_frame, text="未找到相关资源", font=("Microsoft YaHei", 14), text_color=self.colors['text_secondary']).pack(pady=40)
    ...
    def create_list_item(self, item):
        item_frame = ctk.CTkFrame(self.list_frame, corner_radius=10, fg_color=self.colors['item'], border_width=0)
        item_frame.pack(fill="x", padx=2, pady=3)
        item_frame.grid_columnconfigure(1, weight=1)
        item_frame.grid_rowconfigure(2, weight=1)
        
        img_label = ctk.CTkLabel(item_frame, text="Loading", width=55, height=75, fg_color=self.colors['frame'], text_color=self.colors['text_secondary'], corner_radius=4)
        ...
        title_label = ctk.CTkLabel(item_frame, text=item['title'], font=("Microsoft YaHei", 13, "bold"), text_color=self.colors['text_primary'], anchor="w", justify="left", wraplength=320)
        title_label.grid(row=0, column=1, padx=2, pady=(8, 0), sticky="nw")
        
        count_label = ctk.CTkLabel(item_frame, text=item['count'], font=("Microsoft YaHei", 12), text_color=self.colors['text_secondary'], anchor="w")
        count_label.grid(row=1, column=1, padx=2, pady=0, sticky="nw")
        
        btn = ctk.CTkButton(item_frame, text="一键下载", font=("Microsoft YaHei", 12, "bold"), fg_color=self.colors['accent'], hover_color=self.colors['accent_hover'], text_color="#FFFFFF", width=70, height=26, corner_radius=6)
    ...
    def _set_btn_state(self, btn, state):
        ...
        if state == "等待中":
            btn.configure(text="等待中", fg_color=("#E5E5EA", "#3A3A3C"), hover_color=("#D1D1D6", "#2C2C2E"), text_color=("#8E8E93", "#98989D"), state="disabled")
        elif state == "下载中":
            btn.configure(text="下载中", fg_color=self.colors['accent'], hover_color=self.colors['accent_hover'], text_color="#FFFFFF", state="disabled")
        elif state == "继续下载":
            btn.configure(text="继续下载", fg_color=("#FF9500", "#FF9F0A"), hover_color=("#CC7700", "#CC7F08"), text_color="#FFFFFF", state="normal")
        elif state == "一键下载":
            btn.configure(text="一键下载", fg_color=self.colors['accent'], hover_color=self.colors['accent_hover'], text_color="#FFFFFF", state="normal")
        elif state == "下载完成":
            btn.configure(text="下载完成", fg_color=("#E5E5EA", "#3A3A3C"), hover_color=("#D1D1D6", "#2C2C2E"), text_color=("#8E8E93", "#98989D"), state="disabled")
    ...
    def update_pagination(self):
        ...
        if self.current_query and self.total_pages > 0:
            prev_btn = ctk.CTkButton(self.pagination_frame, text="◀", width=36, height=28, font=("Arial", 14, "bold"),
                                     fg_color=self.colors['item'], text_color=self.colors['text_primary'], hover_color=self.colors['bg'],
            ...
            if start_page > 1:
                ...
                if start_page > 2:
                    ctk.CTkLabel(self.pagination_frame, text="...", font=("Arial", 13), text_color=self.colors['text_secondary']).pack(side="left", padx=2)
            
            for p in range(start_page, end_page + 1):
                if p == self.current_page:
                    p_btn = ctk.CTkButton(self.pagination_frame, text=str(p), width=36, height=28, font=("Arial", 13, "bold"),
                                          fg_color=self.colors['accent'], text_color="#FFFFFF", hover_color=self.colors['accent_hover'])
                else:
